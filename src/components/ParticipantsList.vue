...
<template>
  <div v-if="participants?.length" class="divide-y divide-asphalt-light">
    ...
    >
      ...
      <span v-if="showScore && participant[displayField] != null" class="text-snow-dim text-sm font-mono">
        <template v-if="displayField === 'elo'">
          <font-awesome-icon icon="chart-line" class="mr-1" />{{ participant.elo }} ELO
        </template>
        <template v-else>
